 ...
 *       For more info see:
 * https://gitlab.com/cryptsetup/cryptsetup/wikis/TrueCryptOnDiskFormat It
 * operates on full 512 byte sectors and uses CBC with an IV derived from
 * initial key and the sector number. In addition, whitening value is applied on
 * every sector, whitening is calculated from initial key, sector number and
 * mixed using CRC32. Note that this encryption scheme is vulnerable to
 * watermarking attacks and should be used for old compatible containers access
 * only.
 *
...
static int crypt_iv_plain64be_gen(struct crypt_config* cc,
                                  u8* iv,
                                  struct dm_crypt_request* dmreq) {
    memset(iv, 0, cc->iv_size);
...
static int crypt_iv_tcw_ctr(struct crypt_config* cc,
                            struct dm_target* ti,
                            const char* opts) {
    struct iv_tcw_private* tcw = &cc->iv_gen_private.tcw;
    ...
    tcw->crc32_tfm =
        crypto_alloc_shash("crc32", 0, CRYPTO_ALG_ALLOCATES_MEMORY);
...
static int crypt_iv_tcw_gen(struct crypt_config* cc,
                            u8* iv,
                            struct dm_crypt_request* dmreq) {
    struct scatterlist* sg;
    struct iv_tcw_private* tcw = &cc->iv_gen_private.tcw;
...
static int crypt_iv_tcw_post(struct crypt_config* cc,
                             u8* iv,
                             struct dm_crypt_request* dmreq) {
    struct scatterlist* sg;
    u8* dst;
...
static int crypt_iv_random_gen(struct crypt_config* cc,
                               u8* iv,
                               struct dm_crypt_request* dmreq) {
...
static int crypt_iv_eboiv_ctr(struct crypt_config* cc,
                              struct dm_target* ti,
                              const char* opts) {
    if (crypt_integrity_aead(cc)) {
...
static int crypt_iv_eboiv_gen(struct crypt_config* cc,
                              u8* iv,
                              struct dm_crypt_request* dmreq) {
...
static int crypt_iv_elephant_ctr(struct crypt_config* cc,
                                 struct dm_target* ti,
                                 const char* opts) {
    struct iv_elephant_private* elephant = &cc->iv_gen_private.elephant;
    int r;
    ...
    elephant->tfm =
        crypto_alloc_skcipher("ecb(aes)", 0, CRYPTO_ALG_ALLOCATES_MEMORY);
...
static int crypt_iv_elephant(struct crypt_config* cc,
                             struct dm_crypt_request* dmreq) {
    struct iv_elephant_private* elephant = &cc->iv_gen_private.elephant;
    ...
    *(__le64*)es = cpu_to_le64(dmreq->iv_sector * cc->sector_size);
    ...
    if (bio_data_dir(dmreq->ctx->bio_in) != WRITE) {
        ...
        diffuser_cpu_to_disk((__le32*)data_offset,
                             cc->sector_size / sizeof(u32));
    }
...
static int crypt_iv_elephant_gen(struct crypt_config* cc,
                                 u8* iv,
                                 struct dm_crypt_request* dmreq) {
    int r;
...
static int crypt_iv_elephant_post(struct crypt_config* cc,
                                  u8* iv,
                                  struct dm_crypt_request* dmreq) {
    if (bio_data_dir(dmreq->ctx->bio_in) != WRITE)
...
static int crypt_iv_elephant_init(struct crypt_config* cc) {
    ...
    return crypto_skcipher_setkey(elephant->tfm, &cc->key[key_offset],
                                  cc->key_extra_size);
}
...
static int dm_crypt_integrity_io_alloc(struct dm_crypt_io* io,
                                       struct bio* bio) {
    ...
    tag_len =
        io->cc->on_disk_tag_size * (bio_sectors(bio) >> io->cc->sector_shift);

    bip->bip_iter.bi_size = tag_len;
    bip->bip_iter.bi_sector = io->cc->start + io->sector;

    ret =
        bio_integrity_add_page(bio, virt_to_page(io->integrity_metadata),
...
static int crypt_integrity_ctr(struct crypt_config* cc, struct dm_target* ti) {
    ...
    if (crypt_integrity_aead(cc)) {
        ...
        DMDEBUG("%s: Integrity AEAD, tag size %u, IV size %u.",
                dm_device_name(md), cc->integrity_tag_size,
        ...
        DMDEBUG("%s: Additional per-sector space %u bytes for IV.",
                dm_device_name(md), cc->integrity_iv_size);
...
static unsigned int* org_tag_of_dmreq(struct crypt_config* cc,
                                      struct dm_crypt_request* dmreq) {
    u8* ptr =
        iv_of_dmreq(cc, dmreq) + cc->iv_size + cc->iv_size + sizeof(uint64_t);
...
static int crypt_convert_block_aead(struct crypt_config* cc,
    ...
    *sector = cpu_to_le64(ctx->cc_sector - cc->iv_offset);
    ...
    sg_set_page(&dmreq->sg_in[2], bv_in.bv_page, cc->sector_size,
                bv_in.bv_offset);
    ...
    sg_set_page(&dmreq->sg_out[2], bv_out.bv_page, cc->sector_size,
                bv_out.bv_offset);
    ...
    if (bio_data_dir(ctx->bio_in) == WRITE) {
        ...
        if (cc->integrity_tag_size + cc->integrity_iv_size !=
            cc->on_disk_tag_size)
...
static int crypt_alloc_req_skcipher(struct crypt_config* cc,
    ...
    if (!ctx->r.req) {
        ctx->r.req = mempool_alloc(&cc->req_pool,
     ...
     */
    skcipher_request_set_callback(ctx->r.req, CRYPTO_TFM_REQ_MAY_BACKLOG,
                                  kcryptd_async_done,
...
static int crypt_alloc_req_aead(struct crypt_config* cc,
                                struct convert_context* ctx) {
    if (!ctx->r.req_aead) {
        ctx->r.req_aead = mempool_alloc(&cc->req_pool,
                                        in_interrupt() ? GFP_ATOMIC : GFP_NOIO);
        if (!ctx->r.req_aead)
            return -ENOMEM;
    }
     ...
     * requests if driver request queue is full.
     */
    aead_request_set_callback(ctx->r.req_aead, CRYPTO_TFM_REQ_MAY_BACKLOG,
                              kcryptd_async_done,
                              dmreq_of_req(cc, ctx->r.req_aead));
...
static int crypt_alloc_tfms_skcipher(struct crypt_config* cc,
                                     char* ciphermode) {
    unsigned i;
    int err;

    cc->cipher_tfm.tfms =
        kcalloc(cc->tfms_count, sizeof(struct crypto_skcipher*), GFP_KERNEL);
    if (!cc->cipher_tfm.tfms)
        return -ENOMEM;

    for (i = 0; i < cc->tfms_count; i++) {
        cc->cipher_tfm.tfms[i] =
...
static int crypt_alloc_tfms_aead(struct crypt_config* cc, char* ciphermode) {
    ...
    cc->cipher_tfm.tfms_aead[0] =
        crypto_alloc_aead(ciphermode, 0, CRYPTO_ALG_ALLOCATES_MEMORY);
...
static int crypt_setkey(struct crypt_config* cc) {
    ...
    if (crypt_integrity_hmac(cc)) {
        ...
        crypt_copy_authenckey(cc->authenc_key, cc->key,
                              subkey_size - cc->key_mac_size, cc->key_mac_size);
    }

    for (i = 0; i < cc->tfms_count; i++) {
        if (crypt_integrity_hmac(cc))
            r = crypto_aead_setkey(cc->cipher_tfm.tfms_aead[i], cc->authenc_key,
...
static int crypt_ctr_cipher_new(struct dm_target* ti,
                                char* cipher_in,
                                char* key,
                                char** ivmode,
                                char** ivopts) {
    ...
    *ivmode = strrchr(tmp, '-');
    ...
    if (*ivmode && !strcmp(*ivmode, "essiv")) {
        ...
        ret = snprintf(buf, CRYPTO_MAX_ALG_NAME, "essiv(%s,%s)", cipher_api,
                       *ivopts);
...
static int crypt_ctr_cipher_old(struct dm_target* ti,
                                char* cipher_in,
                                char* key,
                                char** ivmode,
                                char** ivopts) {
    ...
    *ivopts = tmp;
    ...
    if (*ivmode && !strcmp(*ivmode, "essiv")) {
        ...
        ret = snprintf(cipher_api, CRYPTO_MAX_ALG_NAME, "essiv(%s(%s),%s)",
                       chainmode, cipher, *ivopts);
    } else {
        ret = snprintf(cipher_api, CRYPTO_MAX_ALG_NAME, "%s(%s)", chainmode,
                       cipher);
    }
    if (ret < 0 || ret >= CRYPTO_MAX_ALG_NAME) {
        kfree(cipher_api);
...
static int crypt_ctr_optional(struct dm_target* ti,
                              unsigned int argc,
                              char** argv) {
    struct crypt_config* cc = ti->private;
    ...
    while (opt_params--) {
        ...
        if (!strcasecmp(opt_string, "allow_discards"))
            ...
            if (!cc->cipher_auth)
                return -ENOMEM;
        } else if (sscanf(opt_string, "sector_size:%hu%c", &cc->sector_size,
                          &dummy) == 1) {
            if (cc->sector_size < (1 << SECTOR_SHIFT) ||
                cc->sector_size > 4096 ||
...
static int crypt_ctr(struct dm_target* ti, unsigned int argc, char** argv) {
    ...
    cc->dmreq_start =
        ALIGN(cc->dmreq_start, __alignof__(struct dm_crypt_request));

    if (align_mask < CRYPTO_MINALIGN) {
        /* Allocate the padding exactly */
        iv_size_padding =
            -(cc->dmreq_start + sizeof(struct dm_crypt_request)) & align_mask;
    } else {
        /*
    ...
    additional_req_size = sizeof(struct dm_crypt_request) + iv_size_padding +
                          cc->iv_size + cc->iv_size + sizeof(uint64_t) +
                          sizeof(unsigned int);

    ret = mempool_init_kmalloc_pool(&cc->req_pool, MIN_IOS,
                                    cc->dmreq_start + additional_req_size);
    if (ret) {
        ti->error = "Cannot allocate crypt request mempool";
        goto bad;
    }

    cc->per_bio_data_size = ti->per_io_data_size = ALIGN(
        sizeof(struct dm_crypt_io) + cc->dmreq_start + additional_req_size,
...
static int crypt_map(struct dm_target* ti, struct bio* bio) {
    ...
    if (cc->on_disk_tag_size) {
        unsigned tag_len =
            cc->on_disk_tag_size * (bio_sectors(bio) >> cc->sector_shift);

        if (unlikely(tag_len > KMALLOC_MAX_SIZE) ||
            unlikely(!(io->integrity_metadata = kmalloc(
                           tag_len, GFP_NOIO | __GFP_NORETRY |
    ...
    switch (type) {
        ...
        case STATUSTYPE_TABLE:
            ...
            if (num_feature_args) {
                ...
                if (cc->on_disk_tag_size)
                    DMEMIT(" integrity:%u:%s", cc->on_disk_tag_size,
                           cc->cipher_auth);
                if (cc->sector_size != (1 << SECTOR_SHIFT))
